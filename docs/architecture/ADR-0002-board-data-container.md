...
2026-06-06 — 起草时对照 UE5.7 DataTable CSV 导入限制（知识缺口 (a)）已实查证伪"CSV 驱动更优"论据。
...
| **Verification Required** | ① UE5.7 编辑器实测：DataTable CSV 导入含 `TArray<int32>` 列是否报 `Unsupported Property type`（已通过文档核验为**仍不支持**，但须项目内编辑器二次确认）；② `UAssetManager` 同步/异步加载 Primary DA 的 5.7 API 签名；③ Blueprint `Floor(float)→int32` 重载在 5.7 的类型推导（关联 OQ-6(d)，属 ADR-0001/`UBoardMathLibrary` 实现层，本 ADR 仅指针） |
...
- **技术**：UE5.7。DataTable CSV 导入**不支持 `TArray` 列**（已核验，见 Decision）；引擎默认 DataTable 是"行集合"，**无顶层/头部字段**概念，棋盘级元数据无处天然落位。
...
1. **`TArray` 列原生支持（决定性论据）。** `FBoardTileData` 含两个 `TArray<int32>` 列。UE5.7 DataTable 的 **CSV 导入不支持 `TArray` 列**（导入报 `Unsupported Property type for struct member`，已核验，见 §Related）。DataTable 仅 **JSON 行导入**能表达数组——但这恰好**抵消了 DataTable 的核心卖点**："经济(5) 调参用 CSV 在 Excel/表格软件编辑更优"。一旦被迫用 JSON，DataTable 相对 Primary DA 已无导入便利优势，反而引入 JSON 手写易错、`FText` 本地化 key 控制受限（NSLOCTEXT 不可用，OQ-7 张力）等新问题。
...
- **关联实现层指针**：`UBoardMathLibrary` 内 F-1 用 `((A%N)+N)%N`、F-2 用通用式 + `Floor(float)→int32` 显式重载（GDD Blueprint 实现约束）——属实现细节，本 ADR 不复述，由实现 story 遵 GDD 约束。Blueprint `Floor` float/double 重载的 5.7 类型推导（知识缺口 (d)）须在实现时实测。
...
- **Cons**：
  - **CSV 不支持 `TArray` 列**（UE5.7 仍报 `Unsupported Property type`，已核验）——`RentTable`/`DiceMultiplierTable` 无法 CSV 导入，**"CSV 调参更优"核心论据被证伪**；被迫改 JSON 行，则手写易错、丧失表格软件优势；
...
- **Rejection Reason**：核心卖点（CSV `TArray` 调参）被引擎限制证伪后，相对 Primary DA 只剩"表格纵览"一项软优势，却引入两资产复杂度、顶层元数据绕路、本地化 key 受限三项实质成本。不划算。
...
|-------------|--------|-------------|--------------------------|
| `design/gdd/board-data.md` | 棋盘数据(#1) | **OQ-6 ⚠ BLOCKING-ADR**：DataTable vs Primary Data Asset 载体裁定 | 选定 Primary Data Asset（`UBoardDataAsset`），核验 CSV 不支持 `TArray` 证伪 DataTable 核心论据 |
