...
//
// Out of Scope（严守 story-001 边界，尽管 ADR Key Interfaces 列了）：
//   - RollDice / FDiceRollResult / 执行序铁律     → story-002
//   - RandomRange 退化契约（Min==Max/Min>Max/2^24） → story-003
//   - lazy-init 兜底种子（!bUseFixedSeed 路径）    → story-004
//   - OnDiceRolled 事件 / FOnDiceRolled delegate   → story-005
...
public:
	// -------------------------------------------------------------------------
	// 4 个 BP-callable 原语（签名冻结，逐字对齐 ADR-0004 §Key Interfaces）
	// -------------------------------------------------------------------------
