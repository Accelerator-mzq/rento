...
> **Implements Pillar**: ① 桌游忠实(经典淘汰制破产)— enables 整局戏剧弧(concept core loop:对手陆续破产,决出唯一赢家)
...
4. **返回裁决**:标记 debtor 待出局(`debtorEliminated=true`)+ 出局序号 + 重算 APC 后的 `winnerId|NONE`(见 CR-6),回合2 据此置 `bIsBankrupt`(player-turn 拥字段)并跳过其回合。
...
| VFX(19)/HUD(16)/音频(22) | `OnPlayerBankrupt(debtor,creditor,reason)` / `OnGameWon(winnerId)` / `OnPlayerRanked(player,rank)` 多播驱动清算/出局/胜利 juice(enable-not-own) | 9→下游 | 9 供事件;下游拥呈现 |
...
**⚠ debtor 显式排除**(本系统 return-only 不直写 bIsBankrupt,APC 须在快照上显式减当前 debtor,不依赖外部写入时序;见 CR-6)。
...
**同 tick 双重出局 tiebreak(systems-designer,防链式破产排名不确定):** 同一 `ResolveBankruptcy` 链内多人出局(A 破产转资产给 B、B 因此 insolvent),决胜序:① **直接债务人优先**(原始 debtor 序号 < 因继承被判 insolvent 者);② 仍并列则 `TurnOrderIndex` 升序。保排名确定性。

| 变量 | 类型 | 范围 | 含义 |
|---|---|---|---|
| `elimination_sequence` | int32 | [1, P-1] | 出局序号(本系统 CR-3 记录) |
...
**双向一致性核查(本节揪出 1 gap,本轮一并改):** economy 列 9→5(is_insolvent 消费 + F-11)✓;property 列 9→6(破产裁决调改 owner)✓;building 列 9→8(in-kind + `LiquidateAllBuildings`)✓——**但 systems-index #9 行 `depends-on 2,5,6` 缺 8**(本系统 CR-3/CR-4 确调 8)→ **本轮补 systems-index #9 → 2,5,6,8**。另:`2→9 orchestration invoke`(回合2 调 `ResolveBankruptcy`,return-only 非硬循环依赖,类比 movement4→经济 push)须在 index 补 orchestration 注;player-turn CR-3.3 已引 9。
...
> **职责边界:enable-not-own。** 本系统只供 `OnPlayerBankrupt(debtor, creditor, reason)` / `OnGameWon(winnerId)` / `OnPlayerRanked(player, rank)` 事件 + 裁决事实;实际 VFX/动画/音效由 VFX(19)/HUD(16)/音频(22) 拥有。本节钉最低呈现契约。
...
- **本系统职责边界**:供事件 + 裁决数据;清算逐格 juice 复用 property `OnOwnershipChanged`,胜利/出局 juice 由本系统 `OnGameWon`/`OnPlayerBankrupt` 使能,呈现归下游。
...
- **AC-3** `[Advisory→Logic]`(CR-5/无环)**GIVEN** 全路径执行,**WHEN** `PlayerState.bIsBankrupt` 字段加写 spy,**THEN** 写入==0(经返回值 `debtorEliminated=true` 由回合2 写;Blueprint-primary 下 Advisory·code-review,C++ 强封装落地后升 Logic,对齐 property-ownership AC-3 模式)。
...
- **AC-29** `[Logic]`(CR-6/F-2/edge fallback)**GIVEN** snapshot=[A]、debtor=A(模拟异常并发态),**WHEN** Resolve,**THEN** dev assert+log;fallback=取 snapshot 中除 debtor 外末名 `bIsBankrupt==false` 者(无则 fatal log),不崩溃、不产 `winnerId=-1` 无效胜者。(纯同步单线程不可达,测 fallback 健壮性。)

### J. 排名口径(CR-7/F-1/F-3)

- **AC-30** `[Logic]`(CR-7/F-3)**GIVEN** 4人局出局序 A=1(先),B=2,C=3,存活 D=胜者,**WHEN** 读所有 `rankingEntry`,**THEN** rank1=D,rank2=C(最晚出局),rank3=B,rank4=A(出局逆序)。
...
- **AC-33** `[Logic]`(F-3 同 tick tiebreak)**GIVEN** 链式破产同 tick:A=直接债务人(原始 debtor),B=因继承资产 insolvent,**WHEN** 读两者 `elimination_sequence`,**THEN** A < B(直接债务人优先);仍并列则按 `TurnOrderIndex` 升序。
...
- **AC-41** `[Logic]`(CR-7/UI)**GIVEN** 4人局出局序已确定,系统9 逐次产 `rankingEntry`,**WHEN** 累计所有 `OnPlayerRanked` 广播,**THEN** P 位玩家中产出 P-1 个(胜者 rank=1 经 OnGameWon 处理),`rank` 字段 2..P 严格递增、无重复、无空缺。
...
**合计 43 条**:39 BLOCKING Logic(含 AC-31/32 共 2 条 CI-stub)+ 3 BLOCKING Integration(AC-38/42/43)+ 1 Advisory(AC-3)。覆盖 CR-1..7 全、F-1..3 全、Edge Cases 全。

## Open Questions

| OQ | 问题 | Owner | 目标解决时机 |
|---|---|---|---|
| **OQ-Bankrupt-1** | 链式破产(A 破产 in-kind 转资产给 B,B 接收后自身 insolvent)的 Rento 行为:B 接收资产后立即判 insolvent 还是变 solvent?APC 是否一次降 2?同 tick 出局序 tiebreak 已定 provisional(直接债务人优先 → `TurnOrderIndex` 升序,F-3),Rento 核查确认。 | game-design / Rento 核查 | 设计冻结前 |
| **OQ-Bankrupt-2** | net_worth 排名口径:本系统采 **nlv 变现值口径**(F-1,= Cash + 经济 F-9,已抵押地=0)而非经典 **face-value**(PurchasePrice + BuildingCost);MVP 不计算(round-cap 禁用),Alpha round-cap 实装前须 Rento 核查定口径(nlv 低估资产多现金少的玩家)。 | Rento 核查 / House Rules23 | Alpha round-cap 实装前 |
