...
- **OQ-VFX-13（R-4 新增）✅ RESOLVED 2026-06-08（producer 裁定）**：`OnPlayerBankrupt`(破产9) vs `OnBankruptcyDeclared`(经济5) 接缝,across bankruptcy9/economy5/HUD16/VFX19。**裁定=维持呈现层（VFX19/HUD16/audio22）统一订经济5 `OnBankruptcyDeclared`**（现金侧时机=破产 juice 语义锚点；放弃 reason 字段，现金侧 juice 不需要）；破产9 `OnPlayerBankrupt` 不被呈现层订阅（防双发）。ADR-0003 §3 收口。V-Own-11/AC-35 + HUD AC-16 现状（订经济5）不变，无须改 Approved GDD。
...
## Review — 2026-06-06 — Verdict: APPROVED_WITH_FOLLOWUPS（收敛终结，R-8）
Scope signal: M
Specialists: systems-designer, qa-lead, game-designer, performance-analyst, technical-artist, creative-director（full 5 specialist + CD synthesis）
verified_blockers: **0**（无真 BLOCKING）
Prior verdict resolved: Yes — R-7 两 must-land（AC-43b 逐 handler `FunctionName` 验证移入 THEN 权威断言 + OQ-VFX-16 掷骰意图事件来源）经主审独立 fresh-grep 复核**全部真落地无残渣**

### 主审独立核实（CD 流程约束:逐条对文核验专家 claim,不接受 claim 当结论）
- **R-7 两 must-land fresh-grep 复核全成立**：AC-43b THEN 已含逐 handler `FunctionName` 匹配断言（抓 ①漏解绑数 owner 系统 6 + ②OQ-VFX-13 改订枚举漂移两类 false-green，固定 12 枚举下可证伪非 vacuous）;OQ-VFX-16 owner/目标期/MVP fallback `OnPhaseChanged(RollPhase)` 齐，V-Own-01 散文注回指正确、未误并入 OQ-VFX-1（hop path）scope。
- **claim-wave 验证（同 R-5/R-6/R-7 模式）**：5 专家提出的 claim 经主审逐条对文核实后**无一条成立为真 BLOCKING**——全部归 finishing-class followup 或 deflate（已裁定项 re-litigate / 已 Approved 姊妹档一致 / Pre-Production 标定范畴 / 渲染层 headless 陷阱）。

### verified_blockers=[]（本轮无真 BLOCKING）
本轮唯一触及主体文本不一致项为 **V-Own-01「点击掷骰后」残留**，主审裁定降为 finishing-class followup（非本轮 BLOCKING），见下 logged decisions。

### logged decisions（finishing-class followup，非 BLOCKING）
1. **V-Own-01 主体文本「点击掷骰后」残留**：L327 澄清注已语义覆盖 CR-1 冲突、无执行分歧;OQ-VFX-16 结算前正确触发源未定不可提前写死=已知待定非已知错误（属正文措辞不一致，finishing-vs-identity 判据:完成已定裁决传播而非重裁系统）。架构期 OQ-VFX-16 裁定后改为「收到掷骰意图事件后」。**可推翻**:若后续认为「实现者只读主体文本会误解触发时机」风险足够高，可要求即把 L326 主体改为「收到掷骰意图事件(OQ-VFX-16 裁定来源)后」并保留澄清注——零跨档 autonomous 机械修，代价极低;当前不强制仅因 OQ-VFX-16 来源未定、写死任一具体事件均可能 stale。
2. **OQ-VFX-13 破产事件接缝（OnPlayerBankrupt vs OnBankruptcyDeclared）维持外置为 producer 跨档债，不单档拦 #19**：#19 V-Own-11 订经济5 `OnBankruptcyDeclared`，已 Approved 的 HUD16 V-Enable-03/AC-16 作同样选择;单档拦 #19 将与已 Approved 姊妹档裁决不一致（[[10px-floor-consistent-with-approved-sibling]] 模式）。逐格清算 juice（V-Own-14b/14c）经 property6 `OnOwnershipChanged` 已兑现 bankruptcy L191 委托，此缝仅剩 V-Own-11 触发源选择。**可推翻**:若 producer 跨档裁定 `OnPlayerBankrupt` 为统一来源，则 #19 AC-43b 枚举列表中 `OnBankruptcyDeclared` 须同步替换为 `OnPlayerBankrupt`（AC-43b 末注已显式登记此维护义务），且 HUD16/economy5 同 PR 一致更新;建议 producer 排入 bankruptcy9/HUD16/economy5 同裁定窗口避免枚举漂移。

### 分歧裁定（CD）
- **CD 裁 APPROVED_WITH_FOLLOWUPS（收敛终结轮）**：R-8 无真 BLOCKING（专家 claim-wave 经主审对文核实零成立），R-7 两 must-land 真落地无残渣，无新正交维度。本轮仅一处主体文本不一致（V-Own-01）降为 finishing-class followup（已知待定 OQ-VFX-16，非已知错误），非 stated-coverage 红线。
- **收敛证据（CD）**：blocker 量逐轮单调降 **5→5→5→4→2→2→2→0** 触底归零;严重度从 R-1 redesign-邻近 → R-4/R-5 第二代残渣 → R-6/R-7 实现约束落地/注释入断言 → R-8 零 BLOCKING + 一措辞 followup。健康收敛触底，**终结轮 = Approved**。

### 遗留 followups（不阻开工，Pre-Production / 架构期）
- **OQ-VFX-11**：`LiquidateAllBuildings` 广播契约接缝（依赖 OQ-Build-3 + bankruptcy9）— producer/架构期。
- **OQ-VFX-13**：`OnPlayerBankrupt` vs `OnBankruptcyDeclared` 接缝 across bankruptcy9/economy5/HUD16/VFX19 — producer 同裁定窗口。
- **OQ-VFX-15**：pawn registry owner（AGameState 内置/movement4/board-manager）— 架构期，门控 AC-51b/51c。
- **OQ-VFX-16**：掷骰意图事件来源（owner 骰子3/回合2，MVP fallback `OnPhaseChanged(RollPhase)`）— 架构期。
- **标定 hard gate**：AC-21 升级闭环（三池 N_active）/ OQ-VFX-3 级联帧预算三补 — #19 story 标定 / 破产9 落地前。
